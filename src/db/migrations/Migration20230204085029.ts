...
export class Migration20230204085029 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table "hero" ("id" serial primary key, "name" varchar(255) not null, "skill" int not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null, "killed_dragons_count" int not null default 0, constraint hero_check check (skill >= 0));',
    );
    this.addSql(
      'alter table "hero" add constraint "hero_name_unique" unique ("name");',
    );

    this.addSql(
      'create table "dragon" ("id" serial primary key, "name" varchar(255) not null, "status" smallint not null default 0, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null, "killed_by_id" int null);',
    );

    this.addSql(
      'create table "permission" ("id" serial primary key, "name" varchar(255) not null);',
    );
    this.addSql(
    ...
    this.addSql(
      'create table "user" ("id" serial primary key, "name" varchar(255) not null, "password" varchar(255) not null, "role_id" int not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null);',
    );
    this.addSql(
    ...
    this.addSql(
      'alter table "dragon" add constraint "dragon_killed_by_id_foreign" foreign key ("killed_by_id") references "hero" ("id") on update cascade on delete set null;',
    );
